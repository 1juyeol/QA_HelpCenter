...
#     ├─ _call_ollama_peak_bucket()       : Ollama Call 2 — 피크 최다 버킷 분석 (데이터 없으면 스킵)
...
async def _call_ollama_peak_bucket(date_str: str, peak_bucket_rows: dict) -> dict:
    ...
    print(f"[Ollama Call 2 - Peak {max_bucket}~{bucket_end}] 프롬프트 길이: {len(prompt)}자")
