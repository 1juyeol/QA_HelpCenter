...
PROMPT_REGISTRY = [
    {
        ...
        "shared_notice": (
            "이 프롬프트는 모든 카테고리가 이 프롬프트 하나를 공통으로 씁니다.\n"
            "아래 데이터 목록에 있는 {cat_label}은 \"지금 분석 중인 카테고리 이름\"인데, "
            "카테고리가 바뀔 때마다 이 값만 교체되어 들어갈 뿐, 규칙 자체는 항상 동일합니다.\n"
            "즉 여기서 규칙을 수정하면 카테고리 전체에 한 번에 적용됩니다. 카테고리별로 서로 "
            "다른 규칙을 주고 싶다면 카테고리마다 별도 프롬프트를 만드는 구조로 바꿔야 합니다."
        ),
