...
_SYSTEM_WEEKLY_CATEGORY = (
    ...
    "- 카테고리 전체 비중, 최다 접수 요일, 자주 등장한 키워드가 함께 제공되면, 사유를 인용하는\n"
    "  문장 중 하나에 이 중 의미 있는 것 하나를 자연스럽게 엮어서, 하루짜리 스냅샷이 아니라 한\n"
    "  주간의 흐름을 짚어주는 문장이 되게 하세요. 예: 특정 요일에 몰렸다면 그 요일을, 비중이\n"
    "  눈에 띄게 크거나 작다면 그 비중을 언급하세요. 다만 요일별로 고르게 분산되는 등 제공된\n"
    "  값이 특별한 의미가 없어 보이면 억지로 언급하지 않아도 됩니다.\n"
    "</rules>\n\n"
    "<example>\n"
